    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-gray-500">
          We couldn&apos;t retrieve the details for order <span className="font-mono">{orderId}</span>.
        </p>
        <Link href="/" className="px-4 py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-900">
          Return to Shop
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto my-12 p-6 md:p-8 bg-white border border-gray-100 shadow-xl rounded-2xl flex flex-col gap-8">
      ...
      <div className="flex flex-col items-center text-center gap-2 border-b border-gray-100 pb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mt-2">Thank you for your order!</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
        ...
        <div className="flex flex-col gap-4 p-5 bg-gray-50 rounded-xl">
          ...
          <div className="flex flex-col gap-1.5 text-gray-600">
            <p><strong>Order ID:</strong> <span className="font-mono text-xs">{order.id}</span></p>
            <p><strong>Date:</strong> {new Date(order.createdAt).toLocaleDateString()}</p>
            <p><strong>Customer:</strong> {order.customerSnapshot.firstName} {order.customerSnapshot.lastName}</p>
      ...
      <div className="flex flex-col gap-4">
        <h3 className="font-semibold text-gray-800 border-b border-gray-100 pb-2">Items Purchased</h3>
          ...
          {order.items.map((item, idx) => (
            <div key={idx} className="flex justify-between items-center text-sm py-2 border-b border-gray-100 last:border-0">
              ...
              <div className="text-right">
                <span className="text-gray-600">{item.quantity} x {order.totals.currency} {item.finalUnitPrice.toFixed(2)}</span>
                <p className="font-semibold text-gray-800 mt-0.5">{order.totals.currency} {item.lineTotal.toFixed(2)}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-3 border-t border-gray-100 pt-6 text-sm">
        <div className="flex justify-between text-gray-500">
          <span>Subtotal</span>
          <span>{order.totals.currency} {order.totals.subtotal.toFixed(2)}</span>
        </div>
        <div className="flex justify-between text-gray-500">
          <span>Shipping Fee</span>
          <span>{order.totals.currency} {order.totals.shipping.toFixed(2)}</span>
        </div>
        <div className="flex justify-between text-gray-500">
          <span>Taxes</span>
          <span>{order.totals.currency} {order.totals.taxes.toFixed(2)}</span>
        </div>
        {order.totals.discount > 0 && (
          <div className="flex justify-between text-green-600 font-medium">
            <span>Discount</span>
            <span>-{order.totals.currency} {order.totals.discount.toFixed(2)}</span>
          </div>
        )}
        <hr className="border-gray-100" />
        <div className="flex justify-between text-base font-bold text-gray-900">
          <span>Total Paid</span>
          <span>{order.totals.currency} {order.totals.total.toFixed(2)}</span>
        </div>
      </div>

      {/* Actions */}
      <div className="flex justify-center border-t border-gray-100 pt-6">
        <Link href="/" className="px-6 py-2.5 bg-gray-800 text-white font-medium rounded-xl hover:bg-gray-900 transition-all duration-300 shadow-md">
          Continue Shopping
        </Link>
      </div>
    </div>
